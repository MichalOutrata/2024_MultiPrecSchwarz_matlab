...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%  FD  %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
nmb_int_gridcols = 40; % number of "unknown" gridcolumns - excluding the left-most and right-most where we have Dirichlet BCs 
...
A=delsqnonsymmetric_Gander(eta,a,b1,b2,G);
RoundType = 'Mmtrx'; if isMmtrx(A), disp('Its an M-matrix, good to go');end
...
function[L,U,p,q_inv,A_diag] = MtrxFactorsLP_Stieltjess(A,d_s)
    ...
    A_rounded = sparse(i,j,v_Rounded);
    A_rounded_OnesDiag = spdiags(ones(size(A,1),1),0, diag(A_diag)\A_rounded);
