function[A_out] = Laplacian(dim,n)
% Inputs:
%   dim (integer)
%   - the dimension of the spaced, dim = 1,2,3
%   n (integer)
%   - the number of **grid points** in each dimension, hence "h=1/(n-1)" && "dim(A_1D)=n-2" 
% Output : 
%   A_out (sparse matrix)
%   - matrix discretize the Laplacian on a n (or n-by-n or n-by-n-by-n) grid on a unit interval/square/cube 
%   - dimensions:  if dim==1 then "(n-2)-by-(n-2)"
%                  if dim==2 then "(n-2)^2-by-(n-2)^2"
%                  if dim==3 then "(n-2)^3-by-(n-2)^3"

h = 1/(n-1); n_int = n-2;
A_1d = spdiags([ones(n_int,1), -2*ones(n_int,1), ones(n_int,1)], [-1,0,1], n_int,n_int);

if dim == 1
    A_out = 1/h^2*A_1d;
elseif dim == 2
    A_2d = kron(A_1d,eye(n-2)) + kron(eye(n-2),A_1d); A_out = 1/h^2*A_2d;
elseif dim == 3
    A_3d = kron(kron(A_1d,eye(n-2)),eye(n-2)) + kron(eye(n-2),kron(A_1d,eye(n-2))) + kron(eye(n-2),kron(eye(n-2),A_1d)); A_out = 1/h^2*A_3d;
end

%%% check errors
% DIMS = n_int*ones(1,dim);
% [~,~,A_NegLaplacNoScaling] = laplacian_downloaded(DIMS); A_correct = (-1)*1/h^2*A_NegLaplacNoScaling;
% err = A_out - A_correct; disp( norm(err,'fro')/norm(A_correct,'fro') );
end
...
function [lambda, V, A] = laplacian_downloaded(varargin)
