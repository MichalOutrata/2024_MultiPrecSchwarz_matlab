...
ProblemChoice = 4;
s_LoadString = append(append('SavedData_ClassicmpSM_Mmtrx_AsPrecs_Prblm',num2str(ProblemChoice)),'.mat'); LoadedData = load(s_LoadString).MyData;
RM_nmbdigits_list = LoadedData{8}; list_of_nmb_int_gridcols = LoadedData{14}; GMRES_nmbiter_Prblm4 = LoadedData{18};
GMRES_resnorms_noprec_Prblm4 = LoadedData{22}; GMRES_nmbittoconv_noprec_Prblm4 = LoadedData{24};
GMRES_resnorms_dAS_Prblm4 = LoadedData{26}; GMRES_nmbittoconv_dAS_Prblm4 = LoadedData{28};
GMRES_resnorms_RAS_Prblm4 = LoadedData{30}; GMRES_nmbittoconv_RAS_Prblm4 = LoadedData{32};
GMRES_resnorms_MS_Prblm4 = LoadedData{34}; GMRES_nmbittoconv_MS_Prblm4 = LoadedData{36};


ProblemChoice = 5;
s_LoadString = append(append('SavedData_ClassicmpSM_Mmtrx_AsPrecs_Prblm',num2str(ProblemChoice)),'.mat'); LoadedData = load(s_LoadString).MyData;
RM_nmbdigits_list = LoadedData{8}; list_of_nmb_int_gridcols = LoadedData{14}; GMRES_nmbiter_Prblm5 = LoadedData{18};
GMRES_resnorms_noprec_Prblm5 = LoadedData{22}; GMRES_nmbittoconv_noprec_Prblm5 = LoadedData{24};
GMRES_resnorms_dAS_Prblm5 = LoadedData{26}; GMRES_nmbittoconv_dAS_Prblm5 = LoadedData{28};
GMRES_resnorms_RAS_Prblm5 = LoadedData{30}; GMRES_nmbittoconv_RAS_Prblm5 = LoadedData{32};
GMRES_resnorms_MS_Prblm5 = LoadedData{34}; GMRES_nmbittoconv_MS_Prblm5 = LoadedData{36};
...
nmb_iters_ToPlot = GMRES_nmbiter_Prblm3; iters_mesh = 0:nmb_iters_ToPlot-1; nmb_DigsToPlot = 6;
...
for ind_nmbdig = nmb_DigsToPlot:-1:1
    PlotData = GMRES_resnorms_dAS_Prblm3{ind_MeshSizeToPlot,ind_nmbdig};
    semilogy(iters_mesh, PlotData(1:nmb_iters_ToPlot),'Color',[MyColors{ind_nmbdig}],'Marker','o','MarkerSize',12,'LineWidth',2); hold on;
end
PlotData = GMRES_resnorms_noprec_Prblm3{ind_MeshSizeToPlot}; semilogy(iters_mesh, PlotData(1:nmb_iters_ToPlot),'Color','k','Marker','o','MarkerSize',12,'LineWidth',2); 
xlabel('iteration','FontSize',24,'interpreter', 'latex'); ylabel('$\|\mathbf{r}_k \| \, / \, \| \mathbf{r}_0 \|$','FontSize',24,'interpreter', 'latex'); title('GMRES preconditioned with dAS','FontSize',24, 'interpreter', 'latex');

nexttile();
for ind_nmbdig = nmb_DigsToPlot:-1:1
    PlotData = GMRES_resnorms_RAS_Prblm3{ind_MeshSizeToPlot,ind_nmbdig}; semilogy(iters_mesh, PlotData(1:nmb_iters_ToPlot),'Color',[MyColors{ind_nmbdig}],'Marker','o','MarkerSize',12,'LineWidth',2); hold on;
end
PlotData = GMRES_resnorms_noprec_Prblm3{ind_MeshSizeToPlot}; semilogy(iters_mesh, PlotData(1:nmb_iters_ToPlot),'Color','k','Marker','o','MarkerSize',12,'LineWidth',2); 
...
nexttile();
for ind_nmbdig = nmb_DigsToPlot:-1:1
    PlotData = GMRES_resnorms_MS_Prblm3{ind_MeshSizeToPlot,ind_nmbdig}; semilogy(iters_mesh, PlotData(1:nmb_iters_ToPlot),'Color',[MyColors{ind_nmbdig}],'Marker','o','MarkerSize',12,'LineWidth',2); hold on;
end
PlotData = GMRES_resnorms_noprec_Prblm3{ind_MeshSizeToPlot}; semilogy(iters_mesh, PlotData(1:nmb_iters_ToPlot),'Color','k','Marker','o','MarkerSize',12,'LineWidth',2);
...
for ind=1:length(t.Children)
    ...
    set(curr_ax,'FontWeight','bold','FontSize',22);
end
...
figure; t(1) = tiledlayout(nmbRowsTiles,nmbColsTiles,'TileSpacing','compact','Padding','tight');

ind_MeshSizeToPlot = length(list_of_nmb_int_gridcols); angle1 = 227; angle2 = 26;


% % first row: convergence curves of dAS/RAS/MS-prec GMRES for different d_s - Prblm 3
% %%%%%
% nexttile(1);
% for ind_nmbdig = nmb_DigsToPlot:-1:1
%     PlotData = GMRES_resnorms_dAS_Prblm3{ind_MeshSizeToPlot,ind_nmbdig};
...
% end
% PlotData = GMRES_resnorms_noprec_Prblm3{ind_MeshSizeToPlot}; semilogy(iters_mesh, PlotData(1:nmb_iters_ToPlot),'Color','k','Marker','o','MarkerSize',12,'LineWidth',2); 
% title('damped additive Schwarz','FontSize',24,'FontWeight','bold', 'interpreter', 'latex'); xlabel('iteration','FontSize',24,'interpreter', 'latex'); 
% 
% nexttile(2);
% for ind_nmbdig = nmb_DigsToPlot:-1:1
%     PlotData = GMRES_resnorms_RAS_Prblm3{ind_MeshSizeToPlot,ind_nmbdig}; semilogy(iters_mesh, PlotData(1:nmb_iters_ToPlot),'Color',[MyColors{ind_nmbdig}],'Marker','o','MarkerSize',12,'LineWidth',2); hold on;
% end
% PlotData = GMRES_resnorms_noprec_Prblm3{ind_MeshSizeToPlot}; semilogy(iters_mesh, PlotData(1:nmb_iters_ToPlot),'Color','k','Marker','o','MarkerSize',12,'LineWidth',2); 
% title('restricted additive Schwarz','FontSize',24,'FontWeight','bold', 'interpreter', 'latex'); xlabel('iteration','FontSize',24,'interpreter', 'latex'); 
% 
% nexttile(3);
% for ind_nmbdig = nmb_DigsToPlot:-1:1
%     PlotData = GMRES_resnorms_MS_Prblm3{ind_MeshSizeToPlot,ind_nmbdig}; semilogy(iters_mesh, PlotData(1:nmb_iters_ToPlot),'Color',[MyColors{ind_nmbdig}],'Marker','o','MarkerSize',12,'LineWidth',2); hold on;
% end
% PlotData = GMRES_resnorms_noprec_Prblm3{ind_MeshSizeToPlot}; semilogy(iters_mesh, PlotData(1:nmb_iters_ToPlot),'Color','k','Marker','o','MarkerSize',12,'LineWidth',2);
% title('multiplicative Schwarz','FontSize',24,'FontWeight','bold', 'interpreter', 'latex'); xlabel('iteration','FontSize',24,'interpreter', 'latex'); 
% 
% % legend
...
% end
% set(groot,'defaultLegendInterpreter','latex');
% Lgnd = legend(LegendHandles,LegendLabels,'Location','northoutside','Orientation','Horizontal'); 
% Lgnd.NumColumns = 11; Lgnd.Layout.Tile = 'North'; fontsize(Lgnd,35,'points');





% second row: are # prec GMRES iter to converge for diff d_s and nmb_int_gridcols - Prblm 3
%%%%%
nexttile(1);
ribbon(GMRES_nmbittoconv_dAS_Prblm3'); view(angle1,angle2);
xticks([]); ylim([0,17]); yticks([1,4,8,12,16]); yticklabels({}); title('damped additive Schwarz','FontSize',24,'FontWeight','bold', 'interpreter', 'latex'); zlabel('Problem 1','FontSize',24,'interpreter', 'latex');

nexttile(2);
ribbon(GMRES_nmbittoconv_RAS_Prblm3'); view(angle1,angle2);
xticks([]); ylim([0,17]); yticks([1,4,8,12,16]); yticklabels({}); zticklabels({}); title('restricted additive Schwarz','FontSize',24,'FontWeight','bold', 'interpreter', 'latex');

nexttile(3);
ribbon(GMRES_nmbittoconv_MS_Prblm3'); view(angle1,angle2);
xticks([]); ylim([0,17]); yticks([1,4,8,12,16]); yticklabels({}); zticklabels({}); title('multiplicative Schwarz','FontSize',24,'FontWeight','bold', 'interpreter', 'latex');
...
%%%%%
nexttile(4);
ribbon(GMRES_nmbittoconv_dAS_Prblm4'); view(angle1,angle2);
xticks([]); ylim([0,17]); yticks([1,4,8,12,16]); yticklabels({}); zlabel('Problem 2','FontSize',24,'interpreter', 'latex');

nexttile(5);
ribbon(GMRES_nmbittoconv_RAS_Prblm4'); view(angle1,angle2);
xticks([]); ylim([0,17]); yticks([1,4,8,12,16]); yticklabels({}); zticklabels({});

nexttile(6);
ribbon(GMRES_nmbittoconv_MS_Prblm4'); view(angle1,angle2);
xticks([]); ylim([0,17]); yticks([1,4,8,12,16]); yticklabels({}); zticklabels({});



% fourth row: are # prec GMRES iter to converge for diff d_s and nmb_int_gridcols - Prblm 5
%%%%%
nexttile(7);
ribbon(GMRES_nmbittoconv_dAS_Prblm5'); view(angle1,angle2);
xticks([]); ylabel('$d_s$','FontSize',24,'interpreter', 'latex'); ylim([0,17]); yticks([1,4,8,12,16]); yticklabels({'1','4','8','12','16'}); ytickangle(0); zlabel('Problem 3','FontSize',24,'interpreter', 'latex');

nexttile(8);
ribbon(GMRES_nmbittoconv_RAS_Prblm5'); view(angle1,angle2);
...
nexttile(9);
ribbon(GMRES_nmbittoconv_MS_Prblm5'); view(angle1,angle2);
xticks([]); ylabel('$d_s$','FontSize',24,'interpreter', 'latex'); ylim([0,17]); yticks([1,4,8,12,16]); yticklabels({'1','4','8','12','16'}); ytickangle(0); zticklabels({});
